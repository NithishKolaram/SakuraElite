    ...
    try {
        ...
        if (result.success) {
            ...
            tbody.innerHTML = result.data.map(reading => {
                const cost = reading.liters_consumed * pricePerLiter;
                return `
                    <tr>
                        ...
                        <td>${reading.liters_consumed} L</td>
...
document.getElementById('calculateWaterBillsBtn').addEventListener('click', async () => {
    if (!confirm('This will calculate water bills based on meter readings and update all units. Continue?')) return;
    ...
    try {
        ...
        if (result.success) {
            showSuccess('Water bills calculated and updated successfully!');
            loadAll();
        } else {
