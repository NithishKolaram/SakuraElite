...
async function loadSummary() {
    try {
        const response = await fetch(`/api/water/summary?monthYear=${encodeURIComponent(currentMonth)}`);
...
document.getElementById('addTankerForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('/api/water/tanker', {
            method: 'POST',
...
window.deleteTanker = async function(id) {
    ...
    try {
        const response = await fetch(`/api/water/tanker/${id}`, {
            method: 'DELETE'
        });
...
window.updateReading = async function(id, field, value) {
    try {
        const response = await fetch(`/api/water/reading/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
document.getElementById('calculateWaterBillsBtn').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('/api/water/calculate-bills', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
